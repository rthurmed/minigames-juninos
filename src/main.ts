import startGame, { Circle, KaboomCtx, Key } from "kaplay"
...
const BALL_Y = GAME_HEIGHT - BALL_V_PADDING
const BALL_MIN_X = BALL_H_PADDING
const BALL_MAX_X = GAME_WIDTH - BALL_H_PADDING
const BALL_STRENGTH_CHANGE_SPEED = 1
const HOLE_RADIUS = BALL_RADIUS * 2 // 1.5
const GRAVITY = 9
const UI_BAR_HEIGHT = GAME_HEIGHT / 2
...
player.onStateEnter("fly", () => {
	const target = GAME_HEIGHT * (1 - strength.getValue())
...
player.onStateEnter("fall", () => {
	...
	const update = game.onUpdate(() => {
		...
		if (player.ball.pos.y > GAME_HEIGHT + (BALL_RADIUS * 2)) {
			update.cancel()
